...
import { HiOutlineUser, HiOutlineShieldCheck, HiOutlineBell, HiOutlineEye, HiOutlineEyeOff, HiOutlineLogout, HiOutlineDocumentText } from 'react-icons/hi'
...
export default function ProfilePage() {
  ...
  // Load my requests when the tab is open
  const { data: myRequests, refetch: refetchMyRequests } = useQuery({
    queryKey: ['my-change-requests'],
    queryFn:  () => profileChangesApi.listMine({ page: 0, size: 20 }),
    enabled:  tab === 'requests',
  })

  const submitChangeRequest = async () => {
    if (!changeNewValue || changeNewValue.trim().length === 0) {
      toast.error('Please enter the new value')
      return
    }
    setChangeBusy(true)
    try {
      await profileChangesApi.create({
      ...
      setChangeNewValue(''); setChangeReason('')
      refetchMyRequests()
    } catch (e) {
  ...
  const TABS = [
    ...
    { key: 'requests',      label: 'Change requests', icon: HiOutlineDocumentText },
        ...
        <div className="space-y-5">
          ...
          <div className="card p-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-base font-semibold">Request a profile change</h3>
            </div>
            <p className="text-sm text-surface-500 mb-4">
              Phone number and address can only be changed by request.
              HR or an Admin will review and approve.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div>
                <label className="label text-xs">Current value</label>
                <input className="input bg-surface-50" disabled
                  value={(changeField === 'phoneNumber' ? user?.phoneNumber : user?.address) || '—'} />
              </div>
            </div>
            <div className="mt-3">
              <label className="label text-xs">New value</label>
              <input className="input"
                placeholder={changeField === 'phoneNumber' ? '+1 555-0100' : '123 Main St, City'}
                value={changeNewValue}
                onChange={e => setChangeNewValue(e.target.value)} />
            </div>
            <div className="mt-3">
              <label className="label text-xs">Reason <span className="text-surface-400 font-normal">(optional)</span></label>
              <textarea className="input" rows={2}
                placeholder="e.g. moved house" value={changeReason}
                onChange={e => setChangeReason(e.target.value)} />
            </div>
            <div className="flex justify-end mt-4">
              <button onClick={submitChangeRequest} disabled={changeBusy}
                className="btn-primary">
                {changeBusy ? 'Submitting…' : 'Submit request'}
              </button>
            </div>
          </div>
          ...
          <div className="card overflow-hidden">
            <div className="px-5 py-4 border-b border-surface-100">
              <p className="text-sm font-semibold text-surface-800">Your past requests</p>
            </div>
            {!myRequests ? (
              <div className="flex justify-center py-8"><Spinner /></div>
            ) : myRequests.content?.length === 0 ? (
              <div className="px-5 py-8 text-center text-sm text-surface-400">No requests yet</div>
            ) : (
                  ...
                  <div key={r.id} className="px-5 py-3 flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-surface-800">
                        {r.fieldName === 'phoneNumber' ? 'Phone number' : 'Address'}
                      </p>
                      <p className="text-xs text-surface-500 mt-0.5">
                        <span className="text-surface-400">From:</span> {r.oldValue || '(empty)'}{' '}
                        <span className="text-surface-400">→ To:</span> <strong>{r.newValue}</strong>
                      </p>
                      {r.reason && <p className="text-xs text-surface-400 mt-0.5 italic">"{r.reason}"</p>}
                      {r.reviewNote && (
