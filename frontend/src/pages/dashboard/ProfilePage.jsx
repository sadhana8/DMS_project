  ...
  return (
    <div className="animate-fade-in max-w-2xl mx-auto">
      ...
      <div className="card p-6 mb-5 flex items-center gap-4">
        <div className="w-16 h-16 rounded-full bg-primary-100 dark:bg-primary-900/40 text-primary-700 dark:text-primary-400 flex items-center justify-center text-xl font-bold flex-shrink-0">{initials}</div>
        <div>
          <h1 className="text-xl font-semibold text-surface-900 dark:text-gray-100">{user?.firstName} {user?.lastName}</h1>
          <p className="text-sm text-surface-500 dark:text-gray-400">{user?.email}</p>
        ...
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={clsx('flex items-center gap-2 px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors',
              tab === t.key ? 'border-primary-600 text-primary-700 dark:text-primary-400' : 'border-transparent text-surface-500 dark:text-gray-400 hover:text-surface-800 dark:hover:text-gray-100')}>
      ...
      {tab === 'profile' && (
        <form onSubmit={profileForm.handleSubmit(onProfile)} className="card p-6 space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-surface-400 dark:text-gray-500 mt-1">Email cannot be changed</p>
          </div>
          <div>
      ...
      {tab === 'notifications' && (
        <div className="card overflow-hidden">
          ...
          {notifSettings.map((s, i) => (
            <div key={s.type} className={`grid grid-cols-[1fr_auto_auto] items-center gap-4 px-5 py-3.5 ${i > 0 ? 'border-t border-surface-100 dark:border-gray-800' : ''}`}>
              <div>
                <p className="text-sm font-medium text-surface-800 dark:text-gray-200 dark:text-gray-200">{s.typeLabel}</p>
          ...
          {user?.resignationEffectiveDate ? (
            <div>
              <div className="flex items-center gap-2 mb-3">
                <div className="w-9 h-9 rounded-lg bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 flex items-center justify-center">
                  <HiOutlineLogout className="w-5 h-5" />
                </div>
                <h3 className="text-base font-semibold dark:text-gray-100">Resignation already submitted</h3>
              </div>
              <p className="text-sm text-surface-600 dark:text-gray-300 dark:text-gray-300 mb-2">
                Your resignation was recorded on{' '}
                <strong>{new Date(user.resignationDate).toLocaleString()}</strong>.
              </p>
              <p className="text-sm text-surface-600 dark:text-gray-300">
                Your DocVault access will end at end of day on{' '}
                <strong>{new Date(user.resignationEffectiveDate).toLocaleDateString()}</strong>.
              </p>
              <p className="text-xs text-surface-400 dark:text-gray-500 mt-3">
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-base font-semibold dark:text-gray-100">Submit resignation</h3>
              </div>
